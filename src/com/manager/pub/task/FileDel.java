...
public class FileDel extends Thread {
	protected static final Logger logger = Logger.getLogger(FileDel.class);
	...
	public void run() {
		while (!Thread.interrupted()
				&&
				(
					Integer.parseInt(DateUtils.getChar12().substring(8, 10)) < Integer.parseInt(SystemConfig.getSystemConfig().getFfmpegEnd())
				 || Integer.parseInt(DateUtils.getChar12().substring(8, 10)) >= Integer.parseInt(SystemConfig.getSystemConfig().getFfmpegBegin())
				)
			  ){
...
//			}
			...
			try
			{
				...
				if(uploadForm!=null)
				{
					...
					logger.info(SystemConfig.getSystemConfig().getFileRoot()+uploadForm.getPlayPath()+" --> "+SystemConfig.getSystemConfig().getFileRoot()+(uploadForm.getPlayPath().replace(".avi", ".flv")));
					aviToFormat.makeFlashbyvideo(new SystemConfig().getSystemConfig().getFormatFactoryPath(), SystemConfig.getSystemConfig().getFileRoot()+uploadForm.getPlayPath(), SystemConfig.getSystemConfig().getFileRoot()+(uploadForm.getPlayPath().replace(".avi", ".flv")));
					uploadForm.setFlvPath(uploadForm.getPlayPath().replace(".avi", ".flv"));
					
					conn.setAutoCommit(false);
					String updateSQL = "update frame_upload set FLV_PATH = ? where UPLOAD_ID = ?";
