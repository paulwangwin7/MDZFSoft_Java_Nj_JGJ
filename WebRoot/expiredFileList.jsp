...
<form id="fileDelForm" name="fileDelForm" action="" method="post">
							<table class="Js_grayBg common_table file_table">
								...
								if(request.getAttribute(Constants.PAGE_INFORMATION)!=null) {
									com.manager.pub.bean.Page pager = (com.manager.pub.bean.Page)request.getAttribute(Constants.PAGE_INFORMATION);
									fileList = (List<UploadForm>)pager.getListObject();
								}
...
<form action="<%=basePath %>userAction.do?method=expiredFileList" method="post" id="hidExpiredForm">
<input type="hidden" name="pageCute" id="Expired_pageCute" />
</form>
<script>
function showExpired(pageCute)
{
	$('#Expired_pageCute').val(pageCute);
	$('#hidExpiredForm').submit();
}
</script>
<table align="center"><tr><td><jsp:include page="common/page.jsp?function=showExpired"></jsp:include></td></tr></table>
						</div>
					</div>
				</div>
		</div>
