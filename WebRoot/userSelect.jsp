...
<div class="error msg" id="userSelectMsg" style="display:none;" onclick="hideObj('userSelectMsg')">Message if login failed</div>
				...
				<form id="userSelectForm" action="<%=basePath %>userAction.do?method=userSelect" method="post">
...
jQuery(function($) {
	...
	{
		//alert(getJqueryArrayStr("selectUserId"));
		//alert($("#userName_"+getJqueryArrayStr("selectUserId")).html())
<%
	if(request.getParameter("changeUser")!=null && request.getParameter("changeUser").equals("0")) {
%>
		parent.document.getElementById("userName").value = $("#userName_"+getJqueryArrayStr("selectUserId")).html();
		parent.document.getElementById("upload_userId").value = getJqueryArrayStr("selectUserId");
<%
	} else {
%>
		parent.document.getElementById("editName").value = $("#userName_"+getJqueryArrayStr("selectUserId")).html();
		parent.document.getElementById("upload_editId").value = getJqueryArrayStr("selectUserId");
<%
	}
%>
		//parent.document.getElementById(assignmentId).value = getJqueryArrayStr("selectUserId");
		//parent.document.getElementById(assignmentName).value = $("#userName_"+getJqueryArrayStr("selectUserId")).html();
		//parent.closeSelectUser();
		try{//上传部分默认的id和name
